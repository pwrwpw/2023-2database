...
declare
    ...
    PRAGMA EXCEPTION_INIT(child_records_exist, -2292);
    duplicate_name EXCEPTION;
    PRAGMA EXCEPTION_INIT(duplicate_name, -1);
    pk_nosearch EXCEPTION;
    PRAGMA EXCEPTION_INIT(pk_nosearch, -2291);
    check_const EXCEPTION;
    PRAGMA EXCEPTION_INIT(check_const, -2290);
begin
    temp_n := test_n;
    for i in test_n.first..test_n.last loop
        ...
        exception
            when duplicate_name then
                dbms_output.put_line(i||' : 중복 이름 예외 발생 !!!');
            when child_records_exist then
                dbms_output.put_line(i||' : 자식 레코드 발견 !!!');
            when null_insert then
                dbms_output.put_line(i||' : null 삽입 불가능 !!!');
            when pk_nosearch then
                dbms_output.put_line(i||' : 부모키가 없습니다 !!!');
            when check_const then
                dbms_output.put_line(i||' : 체크제약 위반 !!!');
        end;
    end loop;
